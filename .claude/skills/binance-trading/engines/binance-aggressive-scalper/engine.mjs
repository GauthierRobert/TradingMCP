// binance-aggressive-scalper — core engine (pure, no network).
...
//      no-taker + price improvement. The win on Binance is the LOWER ABSOLUTE floor: round-trip
//      ~0.26% (2x0.10% + tight spread + buffer) vs Alpaca's ~0.42% maker.
//   2. ATR VOLATILITY GATE — only fire when the expected move (ATR%) is large enough to
//      clear the fee floor, and ideally when volatility is EXPANDING.
...
// safety arm) on real recent bars AND on actual fills, then self-modifies the grid.
// Independent skill: shares the engine design with the Alpaca sibling but its own state.
// Causal throughout (a decision at bar i uses only bars <= i).
