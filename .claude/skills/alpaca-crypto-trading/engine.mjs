...
// ============================================================================
// FARS — Fee-Aware Reversion Scalper (v1, validated on Alpaca 120/180d 2026-06).
// The "must-act / take-profit once it clears 2x fee" idea, made fee-disciplined
// and REGIME-GATED. Long/flat spot only; causal (no lookahead).
//
// Rule:
//   * Reference "fair value" = EMA(refSpan).
//   * MACRO GATE (the part that makes it work): only buy when the slow EMA is
//     RISING over the last 24 bars AND price is above it. In a downtrend the gate
//     stays shut and FARS sits in cash — this is what stops the knife-catching.
//   * ENTRY (flat->long): macro gate open AND price dips >= dipPct below the ref
//     EMA AND RSI(14) < rsiBuy (buy weakness inside an uptrend).
//   * EXIT (long->flat): take-profit at +tp (tp >= 2*round-trip fee, so every
//     completed trade clears 2x the ~0.50% round-trip), OR stop at -stop, OR a
//     maxBars timeout to free idle capital.
//
// VALIDATED FIXED CONFIG (do NOT per-window fit — fitting overfits, see lab.mjs):
//   { refSpan:24, dipPct:0.01, rsiBuy:35, tp:0.03, stop:0.02, maxBars:48, macroGate:true }
//   Net vs cash on Alpaca 1Hour: ETH +6.9%/+8.7% (180/120d), BTC +6.5%/+7.2%;
//   in held-out downtrends the gate goes flat (0-1 trades, no bleed). SOL loses
//   everywhere (wide spread + persistent downtrend) -> EXCLUDE SOL.
export const FARS_CFG = { refSpan: 24, dipPct: 0.01, rsiBuy: 35, tp: 0.03, stop: 0.02, maxBars: 48, macroGate: true, macroSpan: 240 };

function _ema(close, span) {
  const a = 2 / (span + 1), out = new Array(close.length); out[0] = close[0];
  for (let i = 1; i < close.length; i++) out[i] = a * close[i] + (1 - a) * out[i - 1];
  return out;
}
function _rsi(close, idx, P = 14) {
  if (idx < P) return NaN;
  let G = 0, L = 0;
...
// LIVE decision for the latest bar. `position` = { long: bool, entryPx, heldBars }.
// Returns { action: 'BUY'|'SELL'|'HOLD', reason, ...context }. Causal.
export function farsSignal(candles, position = { long: false, entryPx: 0, heldBars: 0 }, cfg = FARS_CFG) {
  const c = { ...FARS_CFG, ...cfg };
  const close = candles.map(x => x.c), i = close.length - 1;
  const ref = _ema(close, c.refSpan), macro = _ema(close, c.macroSpan);
  const px = close[i], rsi = _rsi(close, i);
  const macroUp = i >= 24 && macro[i] > macro[i - 24] && px > macro[i];
  const ctx = { px: +px.toFixed(4), refEma: +ref[i].toFixed(4), macroEma: +macro[i].toFixed(4),
                macroUp, rsi: +(+rsi).toFixed(1), dipFromRef: +((px / ref[i] - 1) * 100).toFixed(2) };
  if (position.long) {
    const up = px / position.entryPx - 1;
    if (up >= c.tp) return { action: 'SELL', reason: `take-profit +${(up*100).toFixed(2)}% >= tp ${(c.tp*100)}% (clears 2x fee)`, ...ctx };
    if (up <= -c.stop) return { action: 'SELL', reason: `stop ${(up*100).toFixed(2)}% <= -${(c.stop*100)}%`, ...ctx };
    if ((position.heldBars ?? 0) >= c.maxBars) return { action: 'SELL', reason: `timeout ${position.heldBars} bars >= ${c.maxBars}; free capital`, ...ctx };
    return { action: 'HOLD', reason: `long, +${(up*100).toFixed(2)}% (tp ${(c.tp*100)}% / stop ${(c.stop*100)}%)`, ...ctx };
  }
  const dip = px <= ref[i] * (1 - c.dipPct), oversold = rsi < c.rsiBuy;
  if (c.macroGate && !macroUp) return { action: 'HOLD', reason: 'macro gate SHUT (slow EMA not rising / price below it) -> stay FLAT, no knife-catching', ...ctx };
  if (dip && oversold) return { action: 'BUY', reason: `gated dip-buy: ${ctx.dipFromRef}% below ref EMA & RSI ${ctx.rsi}<${c.rsiBuy}, macro up`, ...ctx };
  return { action: 'HOLD', reason: `no entry (dip ${dip} / oversold ${oversold}); waiting for a gated dip`, ...ctx };
}
