...
if (cmd === 'tick') {
  ...
  for (const sym of symbols) {
    ...
    if (held) {
      ...
      plan.push({ sym, holding: false, arm: arm.id, action: sig.action, reason: sig.reason, price: last.c, marketOpen: true, order });
    }
  }
  ...
  console.log(JSON.stringify({ ts: new Date().toISOString(), venue: 'alpaca-equity', feed: FEED, marketOpen: anyOpen, equity, activeArm: activeId, ticks, clearDue,
