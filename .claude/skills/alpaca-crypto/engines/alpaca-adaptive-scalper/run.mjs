// CLI for alpaca-adaptive-scalper. Fetches public Alpaca 1Min crypto bars (no auth)
...
if (cmd === 'decide') {
  ...
  const candles = await fetchBars(sym, '1Min', { start: daysAgoISO(3) });          // ~3 days of 1Min = plenty for EMA50
...
if (cmd === 'scan') {
  ...
  console.log(`=== 1Min BUY scan — best trading arm ${armId} (live posture: ${live}${live === 'A0' ? ' = FLAT' : ''}) ===`);
  for (const sym of symbols) {
    const candles = await fetchBars(sym, '1Min', { start: daysAgoISO(3) });
...
if (cmd === 'tick') {
  ...
  for (const sym of symbols) {
    ...
    if (held) {
      ...
      if (sig.action === 'BUY') {
        ...
        if (!gate.allowLong) {
          plan.push({ sym, holding: false, arm: arm.id, action: 'HOLD',
            reason: `charter-gate blocked BUY (${sig.reason}) — ${gate.reason}`, price: last.c, order: null, charterGate: gate });
          continue;
        }
      ...
      plan.push({ sym, holding: false, arm: arm.id, action: sig.action, reason: sig.reason, price: last.c, order, ...(gate ? { charterGate: gate } : {}) });
    }
  }
  ...
  const out = { ts: new Date().toISOString(), equity, activeArm: activeId,
