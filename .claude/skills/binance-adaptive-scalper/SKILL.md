...
description: Fast 5-minute long/flat crypto momentum scalper for Binance spot that learns and re-tunes itself. Each bar it makes a quick BUY/SELL/HOLD call from an EMA-stack + Donchian-breakout + RSI/volume signal; a counterfactual multi-armed bandit scores a grid of parameter "arms" (plus a FLAT arm) on real recent 5m bars AND on actual fills, then self-modifies the grid toward what works. Medium-aggressive, fee-aware. Binance fees (0.10%/fill) are far lower than Alpaca's, so the fee floor is easier to clear. Use for short-term 5-min timing on Binance (BTCUSDT, ETHUSDT, …). SIMULATION-ONLY by default; independent of the Alpaca scalpers.
...
# Adaptive scalper for Binance — fast 5-minute timing that tunes itself

A **self-learning** short-term engine for **Binance spot**. On every 5-minute bar it makes a
...
is Binance-specific: it fetches **public Binance 5m klines** (no API key) and emits order hints that
...
>   to prefer Binance for a 5-minute scalper.
...
rate** — the fee floor is the dominant variable at a 5-minute cadence, exactly as the project's
...
## How a decision is made (fast, causal, 5m) — identical to the Alpaca engine
...
recent 5m window, net-of-fee P&L is the reward; (2) **real** feedback — actual fills logged via
...
```bash
cd .claude/skills/binance-adaptive-scalper
...
node run.mjs learn   "BTCUSDT" 14                                  # re-score arms on real 5m bars + self-modify
...
- **Turnover is the enemy at 5 minutes** — even at Binance's lower fee. Honour the fee floor and the
