...
export function runBacktest(candles, {
  ...
  // --- agentA tunable logic knobs (defaults reproduce original behavior) ---
  edgeMargin = 0,     // require in-sample net equity >= 1 + edgeMargin*(#blocks-frac) else FLAT
  exitMult = 1,       // widen trend exit band: exit when mom < -k*exitMult (>=1 keeps original)
  confirmGate = false,// require macro gate true AND price>price[Lc] (momentum sign) to ENTER trend
} = {}) {
  ...
  const nextPos = (idx, pos, cfg) => {
    ...
    if (cfg.fam === 'trend') {
      if (!gate) return 0;
      const enter = mom > cfg.k;
      return pos === 0 ? (enter ? 1 : 0) : (mom < -cfg.k * exitMult ? 0 : 1);
    } else {
      return pos === 0 ? ((gate && mom < -cfg.k) ? 1 : 0) : (mom > 0 ? 0 : 1);
    }
  };
  ...
  const bestCfg = (endIdx) => {
    ...
    // confidence floor: the winning family must have shown net in-sample edge
    // exceeding (1 + edgeMargin) AND beaten flat after the turnover penalty, else FLAT.
    if (!best || best.score <= 1 || best.equity < 1 + edgeMargin)
      return { cfg: { fam: 'flat' }, score: 1 };
    return best;
  };
  const oracle = () => {
