...
export function runBacktest(candles, {
  ...
  // --- agentC additions (default = original behavior) ---
  macroRise = 0,        // min EMA-rise fraction over `macroLook` blocks required for bull gate
  macroLookBlocks = 1,  // how many blocks back to measure EMA rise (in cpb units)
  mrGate = false,       // if true, mean-reversion also requires price>EMA confirmation strictly
} = {}) {
  ...
  const macroBull = (idx, spanC) => {
    ...
    const e = ema(spanC); const back = Math.max(0, idx - cpb * macroLookBlocks);
    if (!(close[idx] > e[idx])) return false;
    // require EMA rising by at least macroRise fraction over the lookback
    return (e[idx] / e[back] - 1) > macroRise;
  };
  const nextPos = (idx, pos, cfg) => {
    ...
    if (cfg.fam === 'trend') {
      if (!gate) return 0;
      return pos === 0 ? (mom > cfg.k ? 1 : 0) : (mom < -cfg.k ? 0 : 1);
    } else {
